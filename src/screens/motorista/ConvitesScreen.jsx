    ...
    <>
      <div className="only-mobile screen">
        ...
        <div className="content">
          {loading ? <Loading /> : convites.length === 0 ? vazio : convites.map(cardConvite)}
        </div>
      </div>
      <DesktopShell tipo="motorista" active="inicio" onNavigate={onNavigate}>
        <h1 style={{ fontSize: 24, fontWeight: 800, marginBottom: 20 }}>Convites</h1>
        {loading ? <Loading /> : convites.length === 0 ? vazio : (
          <div className="ofertas-grid-desktop">{convites.map(cardConvite)}</div>
        )}
      </DesktopShell>
    </>
  );
