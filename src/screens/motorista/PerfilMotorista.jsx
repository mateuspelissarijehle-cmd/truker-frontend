...
export function PerfilMotorista({ onNavigate }) {
  ...
  const listaLinks = (links, corIcone) => links.map((item, i) => (
    <div key={i} onClick={() => item.screen && onNavigate(item.screen)}
      style={{ display: "flex", alignItems: "center", gap: 14, padding: "13px 16px", borderBottom: i < links.length - 1 ? "1px solid var(--border)" : "none", cursor: item.screen ? "pointer" : "default" }}>
      <div style={{ width: 36, height: 36, borderRadius: 10, background: corIcone, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, flexShrink: 0 }}>{item.icon}</div>
      <div style={{ flex: 1 }}>
  ...
  return (
    ...
    <div className="only-mobile screen">
      ...
      <div className="content">
        ...
        <div className="card" style={{ padding: 0, overflow: "hidden", marginBottom: 12 }}>
          {listaLinks(linksMinhaConta, "var(--gold-light)")}
        </div>
        ...
        <div className="card" style={{ padding: 0, overflow: "hidden", marginBottom: 12 }}>
          {listaLinks(linksAjuda, "var(--surface2)")}
        </div>
    ...
    {/* Desktop: mesmo padrão já provado em PerfilContratante -- perfil vira
        barra lateral fixa, listas de links lado a lado (item 6, 08/09/2026). */}
    <DesktopShell tipo="motorista" active="conta" onNavigate={onNavigate}>
      <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 24, alignItems: "start" }}>
        <div style={{ position: "sticky", top: 16 }}>
          <div className="card" style={{ textAlign: "center", padding: "20px" }}>
            <div style={{ margin: "0 auto 12px", width: 88 }}>
              <Avatar nome={user?.nome} fotoUrl={perfil?.foto_url} logoEmpresaUrl={perfil?.logo_empresa_url} size={88} />
            </div>
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
            {[["💬", "Chat", "chat"], ["⭐", "Avaliações", "avaliacoes"]].map(([icon, label, screen]) => (
              <div key={label} className="card" style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer", margin: 0 }} onClick={() => onNavigate(screen)}>
                <span style={{ fontSize: 20 }}>{icon}</span><span style={{ fontWeight: 600 }}>{label}</span><span style={{ marginLeft: "auto", color: "var(--text2)" }}>›</span>
              </div>
            ))}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--text3)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Minha Conta</div>
              <div className="card" style={{ padding: 0, overflow: "hidden" }}>
                {listaLinks(linksMinhaConta, "var(--gold-light)")}
              </div>
            </div>
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--text3)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Ajuda e Conta</div>
              <div className="card" style={{ padding: 0, overflow: "hidden" }}>
                {listaLinks(linksAjuda, "var(--surface2)")}
              </div>
            </div>
